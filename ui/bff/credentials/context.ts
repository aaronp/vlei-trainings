...
interface CredentialRegistryStore {
  issueCredential(request: IssueCredentialRequest, timeoutMs?: number): Promise<IssueCredentialResponse>;
}

function makeCredentialRegistryStore(): CredentialRegistryStore {
  return {
    async issueCredential(request: IssueCredentialRequest, timeoutMs: number = 2000): Promise<IssueCredentialResponse> {
      return await issueCredentialWithClient(request, timeoutMs);
    }
  };
}
